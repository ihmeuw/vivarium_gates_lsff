...
all necessary requirements as follows::
  ...
  :~$ conda create --name=vivarium_gates_lsff python=3.6
  ...conda will download python and base dependencies...
  :~$ conda activate vivarium_gates_lsff
  (vivarium_gates_lsff) :~$ git clone https://github.com/ihmeuw/vivarium_gates_lsff.git
  ...git will copy the repository from github and place it in your current directory...
  (vivarium_gates_lsff) :~$ cd vivarium_gates_lsff
  (vivarium_gates_lsff) :~$ pip install -e .
...
install hdf tooling from within your environment like so::
  ...
  (vivarium_gates_lsff) :~$ conda install hdf5

The ``(vivarium_gates_lsff)`` that precedes your shell prompt will probably show
...
``src/vivarium_gates_lsff`` package directory:
...
- ``components``
  ...
  the vivarium_gates_lsff project. You should work with the
...
from any directory.::
  ...
  (vivarium_gates_lsff) :~$ make_specs -v
  2020-06-18 18:18:28.311 | 0:00:00.679701 | build_model_specifications:48 - Writing model spec(s) to "/REPO_INSTALLATION_DIRECTORY/vivarium_gates_lsff/src/vivarium_gates_lsff/model_specifications"
...
run simulations by, e.g.::
   ...
   (vivarium_gates_lsff) :~$ simulate run -v /<REPO_INSTALLATION_DIRECTORY>/vivarium_gates_lsff/src/vivarium_gates_lsff/model_specifications/china.yaml
